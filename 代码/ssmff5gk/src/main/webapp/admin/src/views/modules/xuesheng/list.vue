<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','晚归') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="wanguidengjiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'晚归':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','晚归') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="wanguidengjiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'晚归':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','晚归') && contents.tableBtnIcon == 0" type="success" size="mini" @click="wanguidengjiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'晚归':'' }}</el-button>
    ...
    <wanguidengji-cross-add-or-update v-if="wanguidengjiCrossAddOrUpdateFlag" :parent="this" ref="wanguidengjiCrossaddOrUpdate"></wanguidengji-cross-add-or-update>
...
import wanguidengjiCrossAddOrUpdate from "../wanguidengji/add-or-update";
export default {
  data() {
    return {
      banjiOptions: [],
      ...
      wanguidengjiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    wanguidengjiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.wanguidengjiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xuesheng');
      this.$nextTick(() => {
      this.$refs.wanguidengjiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.$http({
